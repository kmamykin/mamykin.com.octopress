...
Running Rails asset pipeline in development mode is quite slow (in fact too slow if you ask me), so for deployment to production you will want to precompile your assets. If a file exists under public/assets it will be served by rails without invoking the asset pipeline and recompiling the file.
...
Once the assets were precompiled, you can either add them to git, and deploy like that, or upload them to S3, and change config.asset_host to point to your S3 bucket host.
